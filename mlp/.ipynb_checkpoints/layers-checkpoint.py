...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic:
            if self.share_across_batch:
                #shape for a single mask
                mask_shape = (1,) + inputs.shape[1:]
            else:
                # Generate a dropout mask with shape equal to the batch
                mask_shape = inputs.shape
            #keep all the values of an array smaller than keep_rate value
            self.mask = self.rng.uniform(0,1,size=mask_shape) < self.incl_prob
            # Broadcasting mask if share_across_batch is True
            mask =  self.mask * inputs 
            return mask
        else:
            #the model does not apply dropout but applies a tranformation to the inputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        #the gradients with respects to outpus need to be masked
        mask_gradient = grads_wrt_outputs * self.mask
