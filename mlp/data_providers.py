...
class EMNISTDataProvider(DataProvider):
    ...
    def label_smoothing(self, int_targets, alpha=0.1):
        ...
        """
        #Assign the length of int_targets to num_rows
        num_rows = int_targets.shape[0]
        K = self.num_classes

        #Initialise vector with zeros
        smoothed_targets = np.zeros((num_rows, K))
    
        for i in range(num_rows):
            # Set all smoothed value
            smoothed_targets[i] = alpha / (K - 1)
            # Set the correct class
            smoothed_targets[i, int_targets[i]] = 1 - alpha  
